...
--
-- Host: 127.0.0.1
-- Generation Time: Sep 13, 2024 at 04:05 PM
...
DROP PROCEDURE IF EXISTS `checkBookingPersonnelAvailability`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `checkBookingPersonnelAvailability` (IN `p_booking_id` INT, IN `p_employee_id` INT)   BEGIN
	SELECT COUNT(*) AS total
...
DROP PROCEDURE IF EXISTS `checkBookingPersonnelViaEmployeeIDExist`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `checkBookingPersonnelViaEmployeeIDExist` (IN `p_booking_id` INT, IN `p_employee_id` INT)   BEGIN
	SELECT COUNT(*) AS total
...
DROP PROCEDURE IF EXISTS `deleteBookingPersonnel`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `deleteBookingPersonnel` (IN `p_booking_personnel_id` INT)   BEGIN
...
DROP PROCEDURE IF EXISTS `generateBookingPersonnelList`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `generateBookingPersonnelList` (IN `p_booking_id` INT)   BEGIN
...
DROP PROCEDURE IF EXISTS `generateEmployeeBookingOptions`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `generateEmployeeBookingOptions` (IN `p_booking_id` INT)   BEGIN
...
DROP PROCEDURE IF EXISTS `getBookingPersonnel`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `getBookingPersonnel` (IN `p_booking_personnel_id` INT)   BEGIN
	SELECT * FROM booking_personnel
	WHERE booking_personnel_id = p_booking_personnel_id;
END$$

DROP PROCEDURE IF EXISTS `getBookingPersonnelViaEmployeeID`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `getBookingPersonnelViaEmployeeID` (IN `p_booking_id` INT, IN `p_employee_id` INT)   BEGIN
	SELECT * FROM booking_personnel
	WHERE booking_id = p_booking_id AND employee_id = p_employee_id;
END$$
...
DROP PROCEDURE IF EXISTS `getEmployeePrimaryAddress`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `getEmployeePrimaryAddress` (IN `p_employee_id` INT)   BEGIN
	SELECT * FROM employee_address
	WHERE employee_id = p_employee_id AND default_address = 'Primary';
END$$
...
DROP PROCEDURE IF EXISTS `insertBookingPersonnel`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `insertBookingPersonnel` (IN `p_booking_id` INT, IN `p_employee_id` INT, IN `p_last_log_by` INT)   BEGIN
...
DROP PROCEDURE IF EXISTS `updateBookingPersonnelJobTime`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `updateBookingPersonnelJobTime` (IN `p_booking_personnel_id` INT, IN `p_job_type` VARCHAR(20), IN `p_last_log_by` INT)   BEGIN    
...
DROP TABLE IF EXISTS `booking_personnel`;
CREATE TABLE `booking_personnel` (
  `booking_personnel_id` int(10) UNSIGNED NOT NULL,
  `booking_id` int(10) UNSIGNED NOT NULL,
...
(2, 'Administrator', 'lawrenceagulto.317@gmail.com', 'ldagulto', 'RYHObc8sNwIxdPDNJwCsO8bXKZJXYx7RjTgEWMC17FY%3D', './components/user-account/image/profile_image/2/HjQG.jpg', 'No', 'Yes', NULL, 0, '2024-09-13 20:59:48', '2025-12-30', 'bi2hXirf%2BQ6cCo6fIklo5ax5USH5Gl41dP2lOSp5yoA%3D', '2024-09-08 19:42:32', 'Yes', 'No', NULL, NULL, 0, NULL, 0, NULL, 'Yes', 'XSG%2BJfV1TXKGpVRLrZqoB9CRGcGXhm1aKi1IMrXLBUo%3D', 'Employee', 'Yes', 1, NULL, NULL, NULL, NULL, '2024-08-21 09:45:47', 2),
...
ALTER TABLE `booking_personnel`
  ...
  ADD KEY `last_log_by` (`last_log_by`),
  ADD KEY `booking_personnel_index_booking_personnel_id` (`booking_personnel_id`),
  ADD KEY `booking_personnel_index_booking_id` (`booking_id`);
...
--
ALTER TABLE `booking_personnel`
  MODIFY `booking_personnel_id` int(10) UNSIGNED NOT NULL AUTO_INCREMENT;
...
--
ALTER TABLE `customer_bank_account`
  MODIFY `customer_bank_account_id` int(10) UNSIGNED NOT NULL AUTO_INCREMENT;
...
--
ALTER TABLE `customer_inquiry`
  MODIFY `customer_inquiry_id` int(10) UNSIGNED NOT NULL AUTO_INCREMENT;
...
--
ALTER TABLE `booking_personnel`
  ADD CONSTRAINT `booking_personnel_ibfk_1` FOREIGN KEY (`booking_id`) REFERENCES `booking` (`booking_id`),
  ADD CONSTRAINT `booking_personnel_ibfk_2` FOREIGN KEY (`last_log_by`) REFERENCES `user_account` (`user_account_id`);
