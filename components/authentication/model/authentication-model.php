...
class AuthenticationModel {
    ...
    #
    # -------------------------------------------------------------
    public function insertPasswordHistory($p_user_id, $p_email, $p_password) {
        $stmt = $this->db->getConnection()->prepare('CALL insertPasswordHistory(:p_user_id, :p_email, :p_password)');
        $stmt->bindValue(':p_user_id', $p_user_id, PDO::PARAM_INT);
        $stmt->bindValue(':p_email', $p_email, PDO::PARAM_STR);
        $stmt->bindValue(':p_password', $p_password, PDO::PARAM_STR);
        $stmt->execute();
