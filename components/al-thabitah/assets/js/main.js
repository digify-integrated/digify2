(function ($) {
    ...
    const waveTextAnimation = (target, options) => {
        ...
        waveText
            .add({ targets: target.querySelectorAll(".anime-text > .word > .char"), opacity: [0, 1], translateY: direction === "down" ? [-20, 0] : [20, 0], delay: anime.stagger(speed ? speed : 50) })
            .add({
                targets: target.querySelectorAll(".anime-text .word .char"),
    ...
    const rotateTextAnimation = (target, options) => {
        ...
        rotateText
            .add({ targets: target.querySelectorAll(".anime-text > .word > .char"), opacity: [0, 1], rotateX: [-70, 0], duration: 150, delay: anime.stagger(speed ? speed : 50), easing: "linear" })
            .add({
                targets: target.querySelectorAll(".anime-text > .word > .char"),
                opacity: content.length > 1 ? 0 : 1,
    ...
    const jumpTextAnimation = (target, options) => {
        ...
        movingLetter9
            .add({ targets: target.querySelectorAll(".anime-text > .word > .char"), scale: [0, 1], duration: 1500, elasticity: 600, transformOrigin: "50% 100%", delay: anime.stagger(speed ? speed : 45, { start: delay }) })
            .add(
                { targets: target.querySelectorAll(".anime-text > .word > .char"), opacity: content.length > 1 ? 0 : 1, scale: content.length > 1 ? 0 : 1, duration: 500, easing: "easeOutExpo", delay: anime.stagger(speed ? speed : 45) },
                `+=${duration - 2300}`
            );
    };
    const zoomTextAnimation = (target, options) => {
        ...
        movingLetter2
            .add({ targets: target.querySelectorAll(".anime-text > .word > .char"), scale: [4, 1], opacity: [0, 1], translateZ: 0, easing: "easeOutExpo", duration: 950, delay: anime.stagger(speed ? speed : 70) })
            .add({ targets: target.querySelectorAll(".anime-text > .word"), opacity: content.length > 1 ? 0 : 1, duration: 1000, easing: "easeOutExpo", delay: 1000 }, `+=${duration - 2500}`);
    };
...
function initMap() {
    $(".map").each(function (e) {
        ...
        const gmap = new google.maps.Map(this, { zoom: 13, center: new google.maps.LatLng(lat, lng), mapTypeId: google.maps.MapTypeId.READMAP, styles: map_style });
        ...
        if (marker !== null && marker !== undefined) {
            if (marker.type.toLowerCase() === "html") {
                ...
                const image_marker = new google.maps.Marker({ icon: { url: marker.src }, position: { lat: lat, lng: lng }, map: gmap, animation: google.maps.Animation.DROP });
                let flag = false;
                if (popup.defaultOpen === true) {
