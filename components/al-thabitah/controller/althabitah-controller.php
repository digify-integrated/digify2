...
# -------------------------------------------------------------
#
# Function: AlthabitahController
# Description: 
# The AlthabitahController class handles work schedule related operations and interactions.
#
# Parameters: None
#
# Returns: None
#
# -------------------------------------------------------------
class AlthabitahController {
    private $althabitahModel;
    ...
    # Description: 
    # The constructor initializes the object with the provided althabitahModel, AuthenticationModel and SecurityModel instances.
    # These instances are used for work schedule related, user related operations and security related operations, respectively.
    #
    # Parameters:
    # - @param AlthabitahModel $althabitahModel     The althabitahModel instance for work schedule related operations.
    ...
    #
    # -------------------------------------------------------------
    public function __construct(AlthabitahModel $althabitahModel, ScheduleTypeModel $scheduleTypeModel, AuthenticationModel $authenticationModel, SecurityModel $securityModel, SystemModel $systemModel) {
        $this->althabitahModel = $althabitahModel;
    ...
    public function addAlthabitah() {
        ...
        if (isset($_POST['work_schedule_name']) && !empty($_POST['work_schedule_name']) && isset($_POST['schedule_type_id']) && !empty($_POST['schedule_type_id'])) {
            $userID = $_SESSION['user_account_id'];
            $althabitahName = $_POST['work_schedule_name'];
            ...
            $althabitahID = $this->althabitahModel->insertAlthabitah($althabitahName, $scheduleTypeID, $scheduleTypeName, $userID);
    
            $response = [
                'success' => true,
                'althabitahID' => $this->securityModel->encryptData($althabitahID),
    ...
    #
    # -------------------------------------------------------------
    public function updateAlthabitah() {
        if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
            return;
        }
        ...
        if (isset($_POST['work_schedule_id']) && !empty($_POST['work_schedule_id']) && isset($_POST['work_schedule_name']) && !empty($_POST['work_schedule_name']) && isset($_POST['schedule_type_id']) && !empty($_POST['schedule_type_id'])) {
            $userID = $_SESSION['user_account_id'];
            $althabitahID = htmlspecialchars($_POST['work_schedule_id'], ENT_QUOTES, 'UTF-8');
            $althabitahName = $_POST['work_schedule_name'];
            $scheduleTypeID = htmlspecialchars($_POST['schedule_type_id'], ENT_QUOTES, 'UTF-8');
        
            $checkAlthabitahExist = $this->althabitahModel->checkAlthabitahExist($althabitahID);
            $total = $checkAlthabitahExist['total'] ?? 0;
            ...
            $this->althabitahModel->updateAlthabitah($althabitahID, $althabitahName, $scheduleTypeID, $scheduleTypeName, $userID);
    ...
    public function saveWorkHours() {
        ...
        if (isset($_POST['work_schedule_id']) && !empty($_POST['work_schedule_id']) && isset($_POST['work_hours_id']) && isset($_POST['day_of_week']) && !empty($_POST['day_of_week']) && isset($_POST['day_period']) && !empty($_POST['day_period']) && isset($_POST['work_from']) && !empty($_POST['work_from']) && isset($_POST['work_to']) && !empty($_POST['work_to']) && isset($_POST['notes'])) {
            ...
            $checkWorkHoursOverlap = $this->althabitahModel->checkWorkHoursOverlap($workHoursID, $althabitahID, $dayOfWeek, $dayPeriod, $workFrom, $workTo);
            ...
            $checkWorkHoursExist = $this->althabitahModel->checkWorkHoursExist($workHoursID);
            $total = $checkWorkHoursExist['total'] ?? 0;

            if($total > 0){
                $this->althabitahModel->updateWorkHours($workHoursID, $althabitahID, $dayOfWeek, $dayPeriod, $workFrom, $workTo, $notes, $userID);
                ...
                $this->althabitahModel->insertWorkHours($althabitahID, $dayOfWeek, $dayPeriod, $workFrom, $workTo, $notes, $userID);
    ...
    #
    # -------------------------------------------------------------
    public function deleteAlthabitah() {
        if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
            return;
        }
        ...
        if (isset($_POST['work_schedule_id']) && !empty($_POST['work_schedule_id'])) {
            ...
            $this->althabitahModel->deleteAlthabitah($althabitahID);
    ...
    public function deleteWorkHours() {
        ...
        if (isset($_POST['work_hours_id']) && !empty($_POST['work_hours_id'])) {
            ...
            $this->althabitahModel->deleteWorkHours($workHoursID);
    ...
    # -------------------------------------------------------------
    #
    # Function: deleteMultipleAlthabitah
    # Description: 
    ...
    #
    # -------------------------------------------------------------
    public function deleteMultipleAlthabitah() {
        if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
            return;
        }

        if (isset($_POST['work_schedule_id']) && !empty($_POST['work_schedule_id'])) {
            $althabitahIDs = $_POST['work_schedule_id'];
    
            foreach($althabitahIDs as $althabitahID){
                $checkAlthabitahExist = $this->althabitahModel->checkAlthabitahExist($althabitahID);
                $total = $checkAlthabitahExist['total'] ?? 0;
    ...
    #
    # -------------------------------------------------------------
    public function getAlthabitahDetails() {
        if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
            return;
        }
        ...
        if (isset($_POST['work_schedule_id']) && !empty($_POST['work_schedule_id'])) {
            ...
            $althabitahDetails = $this->althabitahModel->getAlthabitah($althabitahID);

            $response = [
                'success' => true,
                'althabitahName' => $althabitahDetails['work_schedule_name'] ?? null,
                'scheduleTypeID' => $althabitahDetails['schedule_type_id'] ?? null,
                'scheduleTypeName' => $althabitahDetails['schedule_type_name'] ?? null
            ];
    ...
    public function getWorkHoursDetails() {
        ...
        if (isset($_POST['work_hours_id']) && !empty($_POST['work_hours_id'])) {
            ...
            $checkAlthabitahExist = $this->althabitahModel->checkAlthabitahExist($workHoursID);
            $total = $checkAlthabitahExist['total'] ?? 0;
            ...
            $workHoursDetails = $this->althabitahModel->getWorkHours($workHoursID);
...
$controller = new AlthabitahController(new AlthabitahModel(new DatabaseModel), new ScheduleTypeModel(new DatabaseModel), new AuthenticationModel(new DatabaseModel), new SecurityModel(), new SystemModel());
