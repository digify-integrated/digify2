(function($) {
    ...
    $(function() {
        ...
        if($('#work-locations-table').length){
            workLocationsTable('#work-locations-table');
        }
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            if (response.success) {
                                ...
                                reloadDatatable('#work-locations-table');
                            }
                            else {
        ...
        $(document).on('click','#delete-work-locations',function() {
            let work_locations_id = [];
            const transaction = 'delete multiple work locations';

            $('.datatable-checkbox-children').each((index, element) => {
                if ($(element).is(':checked')) {
                    work_locations_id.push(element.value);
                }
            });
    
            if(work_locations_id.length > 0){
                Swal.fire({
...
function workLocationsTable(datatable_name, buttons = false, show_all = false){
    toggleHideActionDropdown();
    ...
    settings = {
        'ajax': { 
            'url' : 'components/work-locations/view/_work_locations_generation.php',
            'method' : 'POST',
            'dataType': 'json',
            'data': {
                'type' : type,
